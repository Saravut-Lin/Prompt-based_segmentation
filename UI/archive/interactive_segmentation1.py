...
def segment_with_box(image, evt: gr.SelectData):
    ...
    Two-click approach:
      ...
      - Second click => finalize bounding box
        ...
        * else => background (red)
      - Show bounding box in corresponding color
    """
...
def create_ui():
    with gr.Blocks() as demo:
        gr.Markdown("# Pet Segmentation App")
        gr.Markdown(
            "- **Point Prompts**: Each click for Pet/background for segmentation.\n"
            "- **Box Prompts**: Two-click approach:\n"
            "   1) First click = top-left corner (green dot)\n"
            "   2) Second click = bottom-right corner\n"
            "- **Scribble Prompts**: Draw a mask, then press 'Segment with Scribble'."
        )
        ...
        with gr.Tabs():
            ...
            with gr.TabItem("Point Prompts"):
                ...
                gr.Markdown("**Click on the image**. We look at a small 5×5 patch in the default mask to decide if it's pet (>0.5) or background.")
...
if __name__ == "__main__":
    import cv2  # used for drawing circles & rectangles in the box prompt
    demo = create_ui()
